...
function popularCheckboxSection(containerId, itens) {
  ...
  itens.forEach(function (item) {
    ...
    if (item.separador) {
      ...
      sep.className = 'item-separador';
      sep.style.cssText = 'width:100%;height:1px;background:var(--border2);margin:4px 0;pointer-events:none;flex-basis:100%;';
...
function serializarSecao(containerId) {
  ...
  if (!container) return [];
  var itens = [];
  container.querySelectorAll(':scope > .item, :scope > .item-separador').forEach(function (div) {
    if (div.classList.contains('item-separador')) {
