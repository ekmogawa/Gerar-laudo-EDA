...
var _dragState = {
  item:        null,
  placeholder: null,
  lastTarget:  undefined   // evita reordenações desnecessárias
};

function criarPlaceholder(ref) {
  var ph = document.createElement('div');
  ph.className = 'item drag-placeholder';
  ph.style.cssText =
...
function ativarZona(zone) {
  ...
  zone.addEventListener('dragover', function (e) {
    if (!_dragState.item) return;
    e.preventDefault();
    e.dataTransfer.dropEffect = 'move';

    var ph     = _dragState.placeholder;
    var target = getDragAfterElement(zone, e.clientX, e.clientY, ph);

    // Guarda como sentinela: null = "vai pro fim", undefined = "não calculado ainda"
    var key = target || null;
    if (key === _dragState.lastTarget) return;  // posição não mudou — não toca no DOM
    _dragState.lastTarget = key;

    if (target) {
      zone.insertBefore(ph, target);
    } else {
      zone.appendChild(ph);
    }
  });
  ...
  zone.addEventListener('drop', function (e) {
    ...
    if (!ph || !item) return;
    ph.parentElement && ph.parentElement.insertBefore(item, ph);
    ph.remove();
    item.style.opacity = '';
    item.classList.remove('dragging');
    _dragState.item        = null;
    _dragState.placeholder = null;
    _dragState.lastTarget  = undefined;
  });

  zone.addEventListener('dragleave', function (e) {
    if (!zone.contains(e.relatedTarget) && _dragState.placeholder) {
  ...
  zone.addEventListener('dragenter', function (e) {
    if (!_dragState.item) return;
    if (!zone.contains(_dragState.placeholder)) {
      zone.appendChild(_dragState.placeholder);
      _dragState.lastTarget = undefined;
    }
  });
}
...
function ativarItem(item) {
  ...
  item.addEventListener('dragstart', function (e) {
    ...
    _dragState.placeholder = criarPlaceholder(item);
    _dragState.lastTarget  = undefined;

    item.parentElement.insertBefore(_dragState.placeholder, item.nextSibling);
    item.classList.add('dragging');
    e.dataTransfer.effectAllowed = 'move';
    e.dataTransfer.setData('text/plain', '');

    requestAnimationFrame(function () { item.style.opacity = '0.3'; });
  });

  item.addEventListener('dragend', function () {
    item.style.opacity = '';
    item.classList.remove('dragging');
    if (_dragState.placeholder && _dragState.placeholder.parentElement) {
      _dragState.placeholder.parentElement.insertBefore(item, _dragState.placeholder);
      _dragState.placeholder.remove();
    }
    _dragState.item        = null;
    _dragState.placeholder = null;
...
function getDragAfterElement(zone, x, y, exclude) {
  // Coleta itens visíveis, excluindo o sendo arrastado e o placeholder
  var items = Array.from(zone.querySelectorAll('.item:not(.dragging)'))
    .filter(function (el) { return el !== exclude; });

  if (!items.length) return null;

  // Agrupa por linha usando o topo do bounding rect (tolerância de 10px)
  var rows = [];
  items.forEach(function (el) {
    var rect  = el.getBoundingClientRect();
    var rowTop = Math.round(rect.top / 10) * 10;
    var row = rows.find(function (r) { return r.top === rowTop; });
    if (!row) { row = { top: rowTop, bottom: rect.bottom, els: [] }; rows.push(row); }
    row.bottom = Math.max(row.bottom, rect.bottom);
    row.els.push({ el: el, midX: rect.left + rect.width / 2, rect: rect });
  });
  rows.sort(function (a, b) { return a.top - b.top; });

  // Encontra a linha onde o cursor está
  var targetRow = rows[rows.length - 1]; // padrão: última linha
  for (var i = 0; i < rows.length; i++) {
    if (y <= rows[i].bottom) { targetRow = rows[i]; break; }
  }

  // Dentro da linha, encontra o primeiro item à direita do cursor
  var sorted = targetRow.els.slice().sort(function (a, b) { return a.midX - b.midX; });
  for (var j = 0; j < sorted.length; j++) {
    if (x < sorted[j].midX) return sorted[j].el;
  }

  // Cursor está à direita de tudo nesta linha
  // Se há próxima linha, insere antes do primeiro item dela
  var rowIdx = rows.indexOf(targetRow);
  if (rowIdx < rows.length - 1) {
    var nextRow = rows[rowIdx + 1];
    var nextSorted = nextRow.els.slice().sort(function (a, b) { return a.midX - b.midX; });
    return nextSorted[0].el;
  }

  return null; // final absoluto da lista
}
