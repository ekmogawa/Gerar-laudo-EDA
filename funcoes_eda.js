...
async function testarToken() {
  ...
  mostrarToast('🔍 Testando acesso ao repositório…', '#1a2e3a', 10000);
  console.log('[testarToken] Token prefix:', token.substring(0, 15) + '...');

  try {
    // Testa acesso ao repositório
    var repoResp = await fetch(
      'https://api.github.com/repos/' + c.owner + '/' + c.repo,
      { headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json' } }
    );
    var repoData = await repoResp.json();
    console.log('[testarToken] Status repositório:', repoResp.status, repoData);

    if (!repoResp.ok) {
      mostrarToast('❌ Repositório não acessível: ' + (repoData.message || 'HTTP ' + repoResp.status), '#7a1a1a', 8000);
      return;
    }

    var perm = repoData.permissions || {};
    console.log('[testarToken] Permissões:', perm);

    // Testa acesso ao arquivo
    var fileResp = await fetch(
      'https://api.github.com/repos/' + c.owner + '/' + c.repo + '/contents/' + (c.path || 'dados_eda.js') + '?ref=' + (c.branch || 'main'),
      { headers: { 'Authorization': 'token ' + token, 'Accept': 'application/vnd.github+json' } }
    );
    console.log('[testarToken] Status arquivo:', fileResp.status);

    var msg = '✅ Repositório: ' + c.owner + '/' + c.repo;
    msg += ' | push: ' + (perm.push ? 'sim' : 'NÃO');
    msg += ' | arquivo: ' + (fileResp.ok ? 'encontrado' : fileResp.status === 404 ? 'não existe ainda' : 'erro ' + fileResp.status);
    mostrarToast(msg, perm.push ? '#1a3a1a' : '#7a4000', 10000);

    if (!perm.push) {
      console.warn('[testarToken] Token sem permissão de escrita (push: false). Gere um novo token com Contents: Read and write.');
    }
  } catch(e) {
